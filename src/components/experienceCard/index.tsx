...
export default function ExperienceCard({ title, text, image, imageSide }: ExperienceCardProps) {
    return (
        <article className="grid grid-cols-1 md:grid-cols-2 gap-6 px-4 sm:px-10 py-8 items-center">
            ...
            {imageSide === "left" && (
                <img src={image} alt={`Tela do sistema ${title}`} className="w-full object-contain order-1 md:order-2" loading="lazy" />
            )}
        </article>
    );
}